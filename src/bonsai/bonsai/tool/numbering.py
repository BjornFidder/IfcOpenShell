...
class SaveNumber:
    ...
    def get_number(element, settings, numbers_cache=None):
        ...
        if SaveNumber.get_id(element) in numbers_cache:
            return numbers_cache[SaveNumber.get_id(element)]
    ...
    def save_number(ifc_file, element, number, settings, numbers_cache=None):
        ...
        if settings.get("save_type") == "Attribute":
            ...
            numbers_cache[SaveNumber.get_id(element)] = number
            return 1
        if settings.get("save_type") == "Pset":
            pset_name = SaveNumber.get_pset_name(element, settings)
            if not pset_name:
...
class Numbering(bonsai.core.tool.Numbering):
    ...
    def number_elements(
        ...
        for element_number, element in enumerate(elements):
            ...
            if storeys:
                ...
                if storey_number is None and "{S}" in settings.get("format"):
                    if report is not None:
                        report(
                            {"WARNING"},
                            f"Element {getattr(element, 'Name', '')} of type {element.is_a()} with ID {SaveNumber.get_id(element)} is not contained in any storey.",
                        )
                    else:
                        raise Exception(
                            f"Element {getattr(element, 'Name', '')} of type {element.is_a()} with ID {SaveNumber.get_id(element)} is not contained in any storey."
            ...
            if count is None:
                report(
                    {"WARNING"},
                    f"Failed to save number for element {getattr(element, 'Name', '')} of type {element.is_a()} with ID {SaveNumber.get_id(element)}.",
                )
    ...
    @staticmethod
    def remove_numbers(operator, settings, numbers_cache):
        ...
        if not objects:
            operator.report({"WARNING"}, f"No objects selected or available for removal.")
            return {"CANCELLED"}
        ...
        for obj in objects:
            ...
            if element is not None and element.is_a(LoadSelection.get_parent_type(settings)):
                remove_count += SaveNumber.remove_number(ifc_file, element, settings, numbers_cache)
                numbers_cache[SaveNumber.get_id(element)] = None

        if remove_count == 0:
            operator.report({"WARNING"}, f"No elements selected or available for removal.")
            return {"CANCELLED"}

        operator.report({"INFO"}, f"Removed {remove_count} existing numbers.")
        return {"FINISHED"}
